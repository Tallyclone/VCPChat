...
(function () {
  ...
  async function launchVchatApp(appDef) {
    ...
    console.log(
      `[VChatApps] Launching: ${appDef.name} (action: ${appDef.appAction})`
    );
    ...
    try {
      if (desktopApi?.desktopLaunchVchatApp) {
        ...
        if (result?.success) {
          ...
          console.error(
            `[VChatApps] Launch failed: ${appDef.name}`,
            result?.error
          );
          if (window.VCPDesktop.status) {
            window.VCPDesktop.status.update(
  ...
  function injectVchatAppsToDock() {
    ...
    for (const appDef of allApps) {
      ...
      if (existing) {
        ...
        let changed = false;
        if (existing.name !== appDef.name) {
          existing.name = appDef.name;
          changed = true;
        }
        if (existing.emoji !== appDef.emoji) {
          existing.emoji = appDef.emoji;
          changed = true;
        }
        if (existing.description !== appDef.description) {
          existing.description = appDef.description;
          changed = true;
        }
        if (existing.appAction !== appDef.appAction) {
          existing.appAction = appDef.appAction;
          changed = true;
        }
    ...
    if (injectedCount > 0 || updatedCount > 0) {
      console.log(
        `[VChatApps] Dock sync: ${injectedCount} new, ${updatedCount} updated`
      );
      // 触发 Dock 重新渲染
