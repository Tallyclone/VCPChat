...
(function () {
  ...
  function bringToFront(widgetId) {
    ...
    if (Number(widgetData.zIndex) === Number(newZ)) return;
  ...
  function sendToBack(widgetId) {
    ...
    // 设置为低于图标层级的最小值
    const newZ =
      minBelowIcon < Infinity
        ? Math.max(1, minBelowIcon - 1)
        : DESKTOP_ICON_Z - 2;
    if (Number(widgetData.zIndex) === Number(newZ)) return;
    widgetData.zIndex = newZ;
    widgetData.element.style.zIndex = newZ;
  ...
  function moveUp(widgetId) {
    ...
    if (closestAbove) {
      ...
      window.VCPDesktop.widget?.bumpLayoutVersion?.(widgetData);
      window.VCPDesktop.widget?.bumpLayoutVersion?.(closestAbove);
    } else {
      // 没有更高层的挂件，分配新的最高层
  ...
  function moveDown(widgetId) {
    ...
    if (closestBelow) {
      ...
      window.VCPDesktop.widget?.bumpLayoutVersion?.(widgetData);
      window.VCPDesktop.widget?.bumpLayoutVersion?.(closestBelow);
    } else {
      // 没有更低层的挂件，设为最低
