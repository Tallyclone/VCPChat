...
(function () {
  const { CONSTANTS, state, zIndex } = window.VCPDesktop;

  /**
   * 为挂件设置拖拽行为
   * @param {HTMLElement} widgetElement - 挂件容器元素
   * @param {HTMLElement} gripElement - 拖拽手柄元素
   * @param {object} [options] - 拖拽区域配置：grip/widget/custom/none
   */
  function setupDrag(widgetElement, gripElement, options = {}) {
    let isDragging = false;
    let startX = 0;
    let startY = 0;
    let originLeft = 0;
    let originTop = 0;
    const dragRegion = options?.dragRegion || "grip";
    const customSelector =
      typeof options?.dragSelector === "string"
        ? options.dragSelector
        : '[data-drag-region="1"],[data-desktop-drag-region]';

    function getDragTarget() {
      if (dragRegion === "none") return null;
      if (dragRegion === "widget") return widgetElement;
      if (dragRegion === "custom") {
        return widgetElement.querySelector(customSelector) || gripElement;
      }
      return gripElement;
    }
    ...
    widgetElement.dataset.dragRegion = dragRegion;
    ...
    dragTarget.addEventListener("mousedown", (e) => {
      ...
      if (
        ...
        return;
      // 锁定状态下禁止拖拽
      if (window.VCPDesktop.state && window.VCPDesktop.state.desktopLocked)
        return;
      e.preventDefault();
      e.stopPropagation();
    ...
    function onMouseUp() {
      ...
      const leftChanged = parseInt(widgetElement.style.left, 10) !== originLeft;
      const topChanged = parseInt(widgetElement.style.top, 10) !== originTop;
      if ((leftChanged || topChanged) && widgetData) {
        window.VCPDesktop.widget?.bumpLayoutVersion?.(widgetData);
      }
