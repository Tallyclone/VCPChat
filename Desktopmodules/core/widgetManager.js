...
(function () {
  ...
  function bumpWidgetVersion(widgetData) {
    if (!widgetData) return;
    widgetData.widgetVersion = (Number(widgetData.widgetVersion) || 0) + 1;
    bumpDesktopVersion();
  }

  function bumpSourceVersion(widgetData) {
    if (!widgetData) return;
    widgetData.sourceVersion = (Number(widgetData.sourceVersion) || 0) + 1;
    bumpWidgetVersion(widgetData);
  }

  function bumpLayoutVersion(widgetData) {
    if (!widgetData) return;
    widgetData.layoutVersion = (Number(widgetData.layoutVersion) || 0) + 1;
    bumpWidgetVersion(widgetData);
  }

  function applyWidgetFrameOptions(widgetData, frame = {}, options = {}) {
    if (!widgetData || !frame || typeof frame !== "object") return;
    const widgetEl = widgetData.element;
    const grip = widgetEl.querySelector(".desktop-widget-grip");
    const closeBtn = widgetEl.querySelector(".desktop-widget-close-btn");
    const contentWrapper = widgetEl.querySelector(".desktop-widget-content");
    const safeFrame = { ...frame };

    const prevFrameJson = JSON.stringify(widgetData.frameOptions || {});
    if (!widgetData.frameOptions) widgetData.frameOptions = {};

    if (
      safeFrame.showCloseButton === false &&
      safeFrame.contextMenu === false
    ) {
      console.warn(
        "[Desktop] frame.showCloseButton=false and frame.contextMenu=false would make the widget hard to close; keeping context menu enabled."
      );
      safeFrame.contextMenu = true;
    }

    widgetEl.dataset.frameCustomized = "1";

    if (safeFrame.transparent === true) {
      widgetEl.style.background = "transparent";
      widgetEl.style.border = "none";
      widgetEl.style.boxShadow = "none";
    }
    if (safeFrame.background != null)
      widgetEl.style.background = String(safeFrame.background);
    if (safeFrame.border != null)
      widgetEl.style.border = String(safeFrame.border);
    if (safeFrame.shadow != null)
      widgetEl.style.boxShadow = String(safeFrame.shadow);
    if (safeFrame.radius != null)
      widgetEl.style.borderRadius = addCssUnit(safeFrame.radius);
    if (safeFrame.opacity != null)
      widgetEl.style.opacity = String(safeFrame.opacity);
    if (safeFrame.backdropFilter != null)
      widgetEl.style.backdropFilter = String(safeFrame.backdropFilter);

    if (safeFrame.showGrip === false && grip) grip.style.display = "none";
    if (safeFrame.showCloseButton === false && closeBtn)
      closeBtn.style.display = "none";
    if (safeFrame.contentOverflow != null && contentWrapper) {
      contentWrapper.style.overflow = String(safeFrame.contentOverflow);
    }
    if (safeFrame.dragRegion && widgetEl) {
      widgetEl.dataset.dragRegion = String(safeFrame.dragRegion);
    }
    if (
      (safeFrame.dragRegion === "none" || safeFrame.dragRegion === "widget") &&
      grip
    ) {
    ...
    if (
      safeFrame.dragRegion === "custom" &&
      grip &&
      safeFrame.showGrip !== true
    ) {
      grip.style.opacity = "0";
      grip.style.pointerEvents = "none";
    }
    widgetData.frameOptions = safeFrame;
    if (
      options.bumpVersion !== false &&
      prevFrameJson !== JSON.stringify(safeFrame)
    ) {
      bumpLayoutVersion(widgetData);
    }
  }
  ...
  function createWidget(widgetId, options = {}) {
    if (removingWidgetIds.has(widgetId)) {
      console.warn(
        `[Desktop] Widget ${widgetId} is being removed, skipping recreation.`
      );
      return null;
    }
    ...
    applyWidgetFrameOptions(widgetData, options.frame, { bumpVersion: false });

    // 拖拽：frame.dragRegion 支持 grip/widget/custom/none。
    drag.setup(widget, grip, widgetData.frameOptions || {});

    // 右键菜单：必须在 widgetData 初始化和 frame 归一化之后绑定，避免 TDZ/未归一化 frame 风险。
    widget.addEventListener("contextmenu", (e) => {
      if (widgetData.frameOptions?.contextMenu === false) return;
      e.preventDefault();
      e.stopPropagation();
  ...
  function appendWidgetContent(widgetId, fullContent) {
    if (removingWidgetIds.has(widgetId)) {
      console.warn(
        `[Desktop] Widget ${widgetId} is being removed, skipping content append.`
      );
      return;
    }
    ...
    widgetData.cardType = widgetData.cardType || "htmlWidget";
    widgetData.sourceFormat = widgetData.sourceFormat || "inlineHtml";
    widgetData.sourceOrigin =
      widgetData.sourceOrigin === "none"
        ? "stored"
        : widgetData.sourceOrigin || "stored";
    widgetData.sourceCompleteness =
      widgetData.sourceCompleteness === "none"
        ? "partial"
        : widgetData.sourceCompleteness || "partial";
    widgetData.cardSource = fullContent;
    bumpSourceVersion(widgetData);
  ...
  function replaceInWidgets(targetSelector, newContent) {
    ...
    for (const [widgetId, widgetData] of state.widgets) {
      const targetEl =
        widgetData.contentContainer.querySelector(targetSelector);
      if (targetEl) {
        ...
        autoResizeWidget(widgetData);
        console.log(
          `[Desktop] Replaced content in widget ${widgetId}, selector: ${targetSelector}`
        );
        break;
      }
    }
    if (!found) {
      console.warn(
        `[Desktop] Target not found in any widget: ${targetSelector}`
      );
    }
    return found;
  }

  function setWidgetSource(widgetId, source, options = {}) {
    const widgetData = state.widgets.get(widgetId);
    if (!widgetData) return false;
    widgetData.contentBuffer = String(source || "");
    widgetData.cardType = "htmlWidget";
    widgetData.sourceFormat =
      options.sourceFormat || widgetData.sourceFormat || "inlineHtml";
    widgetData.sourceOrigin =
      options.sourceOrigin || widgetData.sourceOrigin || "stored";
    widgetData.sourceCompleteness =
      options.sourceCompleteness || widgetData.sourceCompleteness || "partial";
    widgetData.cardSource = widgetData.contentBuffer;
    widgetData.contentContainer.innerHTML = widgetData.contentBuffer;
    processInlineStyles(widgetData);
  ...
  function cleanupWidgetRuntime(widgetId, widgetData) {
    if (!widgetData) return Promise.resolve();
    const cleanupResult =
      typeof widgetData._cleanup === "function"
        ? Promise.resolve(widgetData._cleanup()).catch((error) => {
            console.warn(
              `[Desktop] Widget cleanup failed for ${widgetId}:`,
              error
            );
          })
        : Promise.resolve();
    ...
    if (widgetData._windowListeners) {
      widgetData._windowListeners.forEach((l) =>
  ...
  function removeWidget(widgetId) {
    ...
    cleanupWidgetRuntime(widgetId, widgetData);
    ...
    const onAnimationEnd = (event) => {
      ...
      if (
        event.animationName &&
        event.animationName !== "desktop-widget-remove"
      )
        return;
    ...
    fallbackTimerId = setTimeout(
      () => finalizeRemove("timeout"),
      REMOVE_FALLBACK_MS
    );
  }

  async function removeWidgetForReload(widgetId) {
    if (removingWidgetIds.has(widgetId)) return false;
    ...
    try {
      await cleanupWidgetRuntime(widgetId, widgetData);
      state.widgets.delete(widgetId);
      bumpDesktopVersion();
      widgetData.element?.remove?.();
      console.log(`[Desktop] Widget removed for reload: ${widgetId}`);
      return true;
    } finally {
      removingWidgetIds.delete(widgetId);
    }
  ...
  function autoResizeWidget(widgetData) {
    ...
    const currentWidth =
      parseInt(widgetData.element.style.width) || CONSTANTS.AUTO_RESIZE_MIN_W;
    ...
    if (
      ...
      const plainText = widgetData.contentBuffer
        .replace(/<[^>]*>/g, " ")
        .replace(/\s+/g, " ")
        .trim();
      if (plainText.length > 0) {
        const estimatedHeight = window.pretextBridge.estimateHeight(
          widgetId,
          plainText,
          "widget",
          currentWidth
        ...
        if (
          Math.abs(estimatedHeight - currentHeight) < 2 &&
          !widgetData.isConstructing
        ) {
    ...
    requestAnimationFrame(() => {
      ...
      const newWidth = Math.min(
        ...
        Math.max(
          CONSTANTS.AUTO_RESIZE_MIN_W,
          contentWidth + CONSTANTS.AUTO_RESIZE_PAD_W
        )
      );
      const newHeight = Math.min(
        window.innerHeight * maxRatio,
        Math.max(
          CONSTANTS.AUTO_RESIZE_MIN_H,
          contentHeight + CONSTANTS.AUTO_RESIZE_PAD_H
        )
      );

      const widget = widgetData.element;
      widget.style.transition = "width 0.15s ease-out, height 0.15s ease-out";
      const oldWidth = widget.style.width;
      const oldHeight = widget.style.height;
      widget.style.width = `${newWidth}px`;
      widget.style.height = `${newHeight}px`;
      if (
        oldWidth !== widget.style.width ||
        oldHeight !== widget.style.height
      ) {
        bumpLayoutVersion(widgetData);
      }
  ...
  function processInlineScripts(widgetData) {
    const scriptElements =
      widgetData.contentContainer.querySelectorAll("script");
    const widgetId = widgetData.element.dataset.widgetId;
      ...
      if (oldScript.src) {
        ...
        if (isLocalOrSameOrigin) {
          ...
          const placeholder = document.createComment(
            `[VCPdesktop] Loading external script: ${scriptUrl}`
          );
          ...
          fetch(scriptUrl)
            ...
            .then((code) => {
              ...
              console.log(
                `[Desktop] External script loaded & sandboxed: ${scriptUrl}`
              );
            })
            .catch((err) => {
              console.warn(
                `[Desktop] Failed to fetch external script: ${scriptUrl}`,
                err.message
              );
              // 回退：直接以原始方式加载（不做沙箱包裹）
          ...
          if (oldScript.crossOrigin)
            newScript.crossOrigin = oldScript.crossOrigin;
          oldScript.replaceWith(newScript);
          console.log(
            `[Desktop] CDN/external script passthrough: ${scriptUrl}`
          );
        }
      } else {
        // 内联脚本：沙箱包裹
        const newScript = document.createElement("script");
        newScript.textContent = buildSandboxCode(
          widgetId,
  ...
  window.VCPDesktop.widget = {
    ...
    replaceInWidgets,
    setSource: setWidgetSource,
    finalize: finalizeWidget,
    remove: removeWidget,
    removeForReload: removeWidgetForReload,
    clearAll: clearAllWidgets,
    autoResize: autoResizeWidget,
    applyFrameOptions: applyWidgetFrameOptions,
    bumpDesktopVersion,
    bumpWidgetVersion,
    bumpSourceVersion,
    bumpLayoutVersion,
