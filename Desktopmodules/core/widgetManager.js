  ...
  function createWidget(widgetId, options = {}) {
    ...
    shadowStyle.textContent = `
            ...
            * { box-sizing: border-box; }
            .widget-inner-content { height: 100%; }
    ...
    // 右键菜单（若 Shadow DOM 内部已 preventDefault 则跳过卡片菜单）
    widget.addEventListener("contextmenu", (e) => {
      const handledInShadow = e.defaultPrevented;
      e.preventDefault();
      e.stopPropagation();
      if (handledInShadow) return;
  ...
  function processInlineScripts(widgetData) {
    ...
    scriptElements.forEach((oldScript) => {
      if (oldScript.src) {
        ...
        const sandboxCode = buildSandboxCode(widgetId, userCode);
        const newScript = document.createElement("script");
        newScript.textContent = sandboxCode;
        oldScript.replaceWith(newScript);
      }
    });
  }
