...
(function () {
  ...
  function initDock() {
    ...
    // 鼠标滚轮横向滚动（横向 Dock 时将纵向滚轮映射为水平滚动）
    dockItemsContainer.addEventListener(
      "wheel",
      (e) => {
        if (!dockItemsContainer.classList.contains("scrollable")) return;
        const pos = state.dock.position || "bottom";
        const isVertical = pos === "left" || pos === "right";
        if (!isVertical && Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
          e.preventDefault();
          dockItemsContainer.scrollLeft += e.deltaY;
        }
      },
      { passive: false }
    );
    ...
    // 滚动时更新边缘渐隐
    dockItemsContainer.addEventListener("scroll", () => {
      updateScrollFade();
    });

    // 窗口大小变化时重新计算 Dock 宽度
    window.addEventListener("resize", () => {
      updateDockOuterWidth();
      // 重新计算滚动区域宽度
      const visibleItems = state.dock.items.filter(
        (item) => item.visible !== false
      );
      const maxVisible = state.dock.maxVisible;
      dockItemsContainer.style.setProperty("--dock-max-visible", maxVisible);
      const needsScroll = visibleItems.length > maxVisible;
      dockItemsContainer.classList.toggle("scrollable", needsScroll);
      updateScrollFade();
    });
  }
  ...
  function renderDock() {
    ...
    // 渲染所有可见项（不截断），超出 maxVisible 的部分通过 CSS 滚动可见
    const visibleItems = state.dock.items.filter(
      (item) => item.visible !== false
    );
    ...
    // 更新"更多"按钮：有图标时始终显示（作为应用抽屉入口）
    const moreBtn = document.getElementById("desktop-dock-more-btn");
    if (moreBtn) {
      const hasHidden = state.dock.items.some((item) => item.visible === false);
      moreBtn.style.display = state.dock.items.length > 0 ? "" : "none";
      const hiddenCount = state.dock.items.filter(
        (item) => item.visible === false
      ).length;
      if (hiddenCount > 0) {
        moreBtn.title = `还有 ${hiddenCount} 个应用未显示`;
      } else {
        moreBtn.title = "全部应用";
      }
    }
  ...
  /**
   * 设置外层 Dock 容器的宽度上限，防止超出屏幕。
   * 图标滚动区域宽度已由 CSS 变量 --dock-max-visible 控制。
   */
  function updateDockOuterWidth() {
    if (!dockElement) return;
    const pos = state.dock.position || "bottom";
    // 纵向 Dock：CSS 已有 max-height: 80vh，无需 JS 干预
    if (pos === "left" || pos === "right") return;
    const edgeDist = state.dock.edgeDistance ?? 12;
    dockElement.style.maxWidth = `${window.innerWidth - edgeDist * 2 - 20}px`;
    dockElement.style.width = ""; // 宽度由 flex 子元素自然撑开
  }
  ...
  function updateScrollFade() {
    ...
    if (!el.classList.contains("scrollable")) return;
    ...
    const atEnd = scrollPos >= scrollSize - 2;

    if (!atStart && !atEnd) {
      el.classList.add("scroll-fade-both");
    } else if (!atStart) {
  ...
  function createDockIcon(item, index) {
    ...
    if (displayIcon) {
      ...
      img.onerror = function () {
        if (item.svgIcon) {
          ...
          this.src !== new URL("../assets/setting.png", location.href).href
        ) {
    ...
    iconWrapper.addEventListener("dragstart", (e) => {
      e.dataTransfer.setData(
        "application/x-desktop-dock-item",
        JSON.stringify(item)
      );
  ...
  function initDockSortDrag() {
    ...
    dockItemsContainer.addEventListener("drop", (e) => {
      ...
      // 移除后索引可能需要调整
      const adjustedTo =
        toActualIndex > fromActualIndex ? toActualIndex - 1 : toActualIndex;
  ...
  function cleanupDragSortIndicators() {
    if (!dockItemsContainer) return;
    const indicators = dockItemsContainer.querySelectorAll(
      ".desktop-dock-sort-indicator"
    );
  ...
  function renderDrawer(filter) {
    ...
    if (filter) {
      ...
      items = items.filter((item) => {
        ...
        const pathMatch =
          item.targetPath &&
          item.targetPath.toLowerCase().includes(lowerFilter);
  ...
  async function launchDockItem(item) {
    ...
    if (lastLaunch && now - lastLaunch < LAUNCH_COOLDOWN_MS) {
      console.log(
        `[Dock] Launch cooldown active for: ${item.name} (${
          LAUNCH_COOLDOWN_MS - (now - lastLaunch)
        }ms remaining)`
      );
      return;
    }
      ...
      try {
        ...
        if (!result.success) {
          ...
          if (window.VCPDesktop.status) {
            window.VCPDesktop.status.update(
              "waiting",
              `启动失败: ${result.error}`
            );
          }
  ...
  function addDockItem(shortcut) {
    const id = `shortcut_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 4)}`;
  ...
  function initFileDrop() {
    ...
    if (canvas) {
      ...
      canvas.addEventListener("drop", async (e) => {
        // 处理 Dock 图标拖入桌面
        const dockItemData = e.dataTransfer.getData(
          "application/x-desktop-dock-item"
        );
        if (dockItemData) {
          e.preventDefault();
  ...
  function createDesktopIcon(item, x, y) {
    ...
    if (item.type === "vchat-app") {
      const existingIcon = canvas.querySelector(
        `.desktop-shortcut-icon[data-app-id="${CSS.escape(item.id)}"]`
      );
      if (existingIcon) {
      ...
      const existingIcon = canvas.querySelector(
        `.desktop-shortcut-icon[data-target-path="${CSS.escape(
          item.targetPath
        )}"]`
      );
      if (existingIcon) {
    ...
    if (item._exactPos) {
      ...
      adjustedY = Math.max(
        CONSTANTS.TITLE_BAR_HEIGHT + 4,
        Math.min(y - 32, window.innerHeight - 120)
      );
    }
    ...
    iconEl.addEventListener("click", (e) => {
      ...
      // 清除其他选中
      canvas
        .querySelectorAll(".desktop-shortcut-icon.selected")
        .forEach((el) => {
    ...
    iconEl.addEventListener("mousedown", (e) => {
      ...
      const onUp = () => {
        ...
        if (isDragging) {
          ...
          const iconState = state.desktopIcons.find(
            (i) =>
      ...
      if (window.VCPDesktop.iconPicker) {
        window.VCPDesktop.iconPicker.open((iconData) => {
          ...
          if (iconData.iconType === "html" && iconData.htmlContent) {
            ...
            // 同步更新桌面上的同源图标 DOM
            updateDesktopIconsByTargetHtml(
              item.targetPath,
      ...
      if (window.VCPDesktop.iconPicker) {
        window.VCPDesktop.iconPicker.open((iconData) => {
          ...
          if (iconData.iconType === "html" && iconData.htmlContent) {
            ...
            if (iconState) {
              iconState.icon = null;
              iconState.htmlIcon = iconData.htmlContent;
            }
            // 替换当前桌面图标 DOM 中的图标元素
            replaceDesktopIconElement(iconEl, "html", iconData.htmlContent);
            // 同步 Dock
            const dockItem = state.dock.items.find(
              (i) =>
                (targetPath && i.targetPath === targetPath) ||
                (appId && i.id === appId)
            );
            if (dockItem) {
              dockItem.icon = null;
              dockItem.htmlIcon = iconData.htmlContent;
              renderDock();
              saveDockConfig();
            }
            // 自动保存桌面图标布局
    ...
    removeBtn.addEventListener("click", () => {
      ...
      iconEl.addEventListener(
        ...
        () => {
          ...
          const idx = state.desktopIcons.findIndex(
            (i) =>
  ...
  async function handleRestoreIcon(item, iconEl, isDock) {
    ...
    if (
      ...
      // 1. 更新 Dock 状态
      const dockItem = state.dock.items.find(
        (i) => i.id === appId || (targetPath && i.targetPath === targetPath)
      );
      if (dockItem) {
  ...
  function updateDesktopIconsByTarget(targetPath, newIconSrc) {
    ...
    const icons = canvas.querySelectorAll(
      `.desktop-shortcut-icon[data-target-path="${CSS.escape(targetPath)}"]`
    );
  ...
  function replaceDesktopIconElement(iconEl, type, content) {
    ...
    const oldPlaceholder = iconEl.querySelector(
      ".desktop-shortcut-icon-placeholder"
    );
  ...
  function applyDockPosition(position, edgeDistance) {
    ...
    switch (pos) {
      ...
      case "left":
        ...
        if (dockItemsContainer)
          dockItemsContainer.style.flexDirection = "column";
        break;
      case "right":
  ...
  async function saveDesktopIcons() {
    ...
    try {
      ...
      console.log(
        `[Dock] Desktop icons saved: ${state.desktopIcons.length} icons`
      );
    } catch (err) {
  ...
  async function restoreDesktopIcons() {
    ...
    try {
      ...
      const savedIcons =
        result.data.currentDesktopIcons || result.data.desktopIcons;
      ...
      if (canvas) {
        canvas
          .querySelectorAll(".desktop-shortcut-icon")
      ...
      console.log(
        `[Dock] Desktop icons restored: ${state.desktopIcons.length}`
      );
    } catch (err) {
      _isRestoringIcons = false;
