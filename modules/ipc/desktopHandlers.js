...
function cleanupStandaloneAppProcesses() {
  for (const [appDir, child] of standaloneAppProcesses.entries()) {
    ...
    try {
      child.kill();
      console.log(
        `[DesktopHandlers] Requested standalone app shutdown: ${appDir} (PID: ${child.pid})`
      );
    } catch (error) {
      console.warn(
        `[DesktopHandlers] Failed to stop standalone app ${appDir}:`,
        error.message
      );
    }
  }
    ...
    try {
      ...
      if (appAction === "open-powershell-executor-terminal") {
        ...
        if (typeof powerShellExecutor.openGuiTerminal !== "function") {
          return {
            success: false,
            error: "PowerShellExecutor GUI entry is not available.",
          };
        }
  ...
  desktopWindow.once("ready-to-show", () => {
    ...
    if (desktopGlobalSettings.alwaysOnBottom) {
      ...
      desktopWindow.once("closed", () =>
        clearTimeout(enableAlwaysOnBottomTimer)
      );
    }
