...
async function handleDesktopRemoteControl(commandPayload) {
  try {
    ...
    switch (command) {
      ...
      case "QueryWidget":
        return await _handleQueryWidget(commandPayload, desktopWin);
      case "ViewWidgetSource":
        return await _handleViewWidgetSource(commandPayload, desktopWin);
      case "EditWidget":
      case "DESKTOP_WIDGET_EDIT":
        return await _handleEditWidget(commandPayload, desktopWin);
      ...
      case "NativeFileMountAction":
        return await _handleNativeFileMountAction(commandPayload, desktopWin);
      case "QueryNativeFileMounts":
        return await _handleQueryNativeFileMounts(desktopWin);
      default:
...
async function _handleQueryWidget(commandPayload, desktopWin) {
  const { widgetId, level = "summary" } = commandPayload;
  if (!widgetId) {
    throw new Error("widgetId parameter is required for QueryWidget.");
  }
  if (!desktopWin || desktopWin.isDestroyed()) {
    throw new Error("Desktop window is not available.");
  }
  ...
  const responseData = await requestDesktopRemote(
    ...
    {
      timeoutMs: 5000,
      timeoutMessage: "Timed out while querying widget.",
    }
  );

  let mdReport = `### Widget Query: \`${widgetId}\`\n\n`;
  mdReport += `- level: ${responseData?.level || level}\n`;
  mdReport += `- type: ${responseData?.type || "unknown"}\n`;
  mdReport += `- sourceFormat: ${responseData?.sourceFormat || "none"}\n`;
  mdReport += `- sourceVersion: ${responseData?.sourceVersion || "-"}\n`;
  mdReport += `- widgetVersion: ${responseData?.widgetVersion || "-"}\n\n`;
  mdReport += "```json\n";
  mdReport += JSON.stringify(responseData || {}, null, 2);
  mdReport += "\n```";
...
async function _handleEditWidget(commandPayload, desktopWin) {
  const payload = commandPayload.payload || commandPayload;
  const widgetId = payload.widgetId || payload.id;
  if (!widgetId) {
    throw new Error("widgetId parameter is required for EditWidget.");
  }

  const targetWin = await ensureDesktopWindow(desktopWin, {
    waitAfterOpenMs: 1000,
    errorMessage: "Unable to open desktop window for widget edit.",
  });
  ...
  const responseData = await requestDesktopRemote(
    ...
    payload,
    {
      timeoutMs: commandPayload.timeoutMs || 30000,
      timeoutMessage: "Timed out while editing widget.",
    }
  );

  let mdReport = `### Widget Edit: \`${widgetId}\`\n\n`;
  mdReport += "```json\n";
  mdReport += JSON.stringify(responseData || {}, null, 2);
  mdReport += "\n```";
...
async function _handleNativeFileMountAction(commandPayload, desktopWin) {
  const targetWin = await ensureDesktopWindow(desktopWin, {
    waitAfterOpenMs: 1000,
    errorMessage: "Unable to open desktop window for NativeFileMount action.",
  });
  const responseData = await requestDesktopRemote(
    targetWin,
    "NativeFileMountAction",
    commandPayload.payload || commandPayload,
    {
      timeoutMs: commandPayload.timeoutMs || 120000,
      timeoutMessage:
        "Timed out while waiting for NativeFileMount action confirmation.",
    }
  );
  return {
    status: "success",
...
async function _handleQueryNativeFileMounts(desktopWin) {
  const targetWin = await ensureDesktopWindow(desktopWin, {
    waitAfterOpenMs: 1000,
    errorMessage: "Unable to open desktop window for NativeFileMount query.",
  });
  const responseData = await requestDesktopRemote(
    targetWin,
    "QueryNativeFileMounts",
    {},
    {
      timeoutMs: 8000,
      timeoutMessage: "Timed out while querying NativeFileMounts.",
    }
  );
  return {
    status: "success",
