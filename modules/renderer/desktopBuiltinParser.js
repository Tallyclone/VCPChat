...
const CUSTOM_UI_FIELDS = new Set([
  "html",
  "css",
  "js",
  "htmlContent",
...
const FIELD_ALIASES = new Map([
  ...
  ["readonly", "mode"],
  ["view", "initialView"],
  ["variant", "ui.layout.variant"],
  ["layout", "ui.layout.view"],
  ["ui.layout.variant", "ui.layout.view"],
  ["ui.toolbar", "ui.actions.toolbar"],
  ["toolbar", "ui.actions.toolbar"],
  ["ui.contextMenu", "ui.actions.contextMenu"],
  ["contextMenu", "ui.actions.contextMenu"],
]);

const UI_PREFIX_ALIASES = [
  ["theme.", "ui.theme."],
  ["layout.", "ui.layout."],
  ["actions.", "ui.actions."],
  ["icons.", "ui.icons."],
...
function stripWrappedValue(value) {
  ...
  if (
    text.startsWith(START_ESCAPE_MARKER) &&
    text.endsWith(END_ESCAPE_MARKER)
  ) {
...
const ARRAY_VALUE_FIELDS = new Set([
  "ui.actions.toolbar",
  "ui.actions.contextMenu",
  "ui.confirm.buttonOrder",
  "ui.agentUI.preview.buttons",
  "ui.agentUI.execution.buttons",
  "ui.agentUI.previewButtons",
  "ui.agentUI.executionButtons",
]);

function shouldCoerceCommaArray(key) {
  return (
    ARRAY_VALUE_FIELDS.has(key) ||
    /\.(toolbar|contextMenu|buttonOrder|buttons)$/i.test(key)
  );
}

function coerceValue(rawValue, key = "") {
  const value = stripWrappedValue(rawValue);
  const trimmed = String(value).trim();
  if (/^(true|false)$/i.test(trimmed)) return trimmed.toLowerCase() === "true";
  if (/^(null|none)$/i.test(trimmed)) return null;
  if (/^-?\d+(?:\.\d+)?$/.test(trimmed)) return Number(trimmed);
  if (trimmed.includes(",") && shouldCoerceCommaArray(key)) {
    return trimmed
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean);
...
function normalizeFieldName(rawKey) {
  ...
  const directAlias = FIELD_ALIASES.get(key);
  if (directAlias) return directAlias;
  for (const [prefix, targetPrefix] of UI_PREFIX_ALIASES) {
    if (key.startsWith(prefix))
      return `${targetPrefix}${key.slice(prefix.length)}`;
  }
  return key;
}
...
function parseDesktopBuiltinFields(rawText) {
  ...
  while (index < lines.length) {
    ...
    if (blockMatch) {
      ...
      if (!closed) {
        throw new Error(
          `DESKTOP_BUILTIN field "${key}" block is not closed with ${endMarker}`
        );
      }
    ...
    if (!kvMatch) continue;
    const key = normalizeFieldName(kvMatch[1]);
    fields.push([key, coerceValue(kvMatch[2], key)]);
  }
...
function applyField(payload, key, value) {
  ...
  // Preserve unknown top-level fields under config for forward-compatible builtin options.
  payload.config[key] = value;
}

function normalizeDesktopBuiltinPayload(payload) {
  if (!payload.type) {
    throw new Error("DESKTOP_BUILTIN requires type");
  }
  ...
  if (
    payload.config.initialView &&
    !["list", "grid", "compact"].includes(payload.config.initialView)
  ) {
  ...
  if (payload.customUI) {
    ...
    if (payload.customUI.fallbackToDefault == null)
      payload.customUI.fallbackToDefault = true;
  }
...
function getDesktopBuiltinLabel(
  rawText,
  ...
  try {
    ...
    return `桌面内置挂件创建请求已发送：${escapeHtml(
      payload.type || "builtin"
    )}`;
  } catch (error) {
    return `桌面内置挂件创建请求解析失败：${escapeHtml(
      error.message || error
    )}`;
  }
}
