...
const THINK_START_REGEX = /<think(?:ing)?>/gi;
const THINK_END_REGEX = /<\/think(?:ing)?>/gi;
...
const STREAM_BLOCK_TAG_REGEX =
  /^(P|DIV|UL|OL|LI|PRE|BLOCKQUOTE|H[1-6]|TABLE|TR|FIGURE)$/;
...
function hasAnyAttribute(el, attrNames) {
  return (
    !!el?.hasAttribute &&
    attrNames.some((attrName) => el.hasAttribute(attrName))
  );
}
...
function parseStreamTail(text) {
  ...
  return refs.markedInstance?.parse
    ? refs.markedInstance.parse(processedText)
    : processedText;
}
...
function ensureStreamingRoots(contentDiv) {
  ...
  let stableBlocksRoot = contentDiv.querySelector(
    ".vcp-stream-stable-blocks-root"
  );
...
function createStableBlockRecord(
  segmentState,
  start,
  end,
  source,
...
function appendStableBlockFragment(
  stableBlocksRoot,
  segmentState,
  sourceText,
  html,
  ...
  const { messageId = null, settings = null } = options;
  ...
  if (typeof refs.renderPostProcessedHtml === "function") {
    ...
    if (enrichResult && typeof enrichResult.catch === "function") {
      enrichResult.catch((error) =>
        console.error("[StreamManager] Stable block enrichment failed:", error)
      );
    }
  } else {
...
function appendNewStableRange(
  ...
  while (segmentState.stableRenderedCutoff < nextStableCutoff) {
    ...
    const markerIndex = findNextLineOnlyToken(
      textForRendering,
      BURST_MARKER_TOKEN,
      currentOffset
    );
    const effectiveMarkerIndex =
      markerIndex !== -1 && markerIndex < nextStableCutoff ? markerIndex : -1;
    const sliceEnd =
      effectiveMarkerIndex === -1 ? nextStableCutoff : effectiveMarkerIndex;
    const sourceText = textForRendering.slice(currentOffset, sliceEnd);

    if (sourceText) {
      const html = parseFullStreamContent(sourceText);
      const blockRecord = appendStableBlockFragment(
        stableBlocksRoot,
        segmentState,
        sourceText,
        html,
    ...
    segmentState.stableRenderedCutoff =
      effectiveMarkerIndex + BURST_MARKER_TOKEN.length;
  }
...
function unwrapStableBlockContainersForBurst(stableBlocksRoot, segmentState) {
  ...
  const blockEls = Array.from(
    stableBlocksRoot.querySelectorAll(":scope > .vcp-stream-stable-block")
  );
...
function ensureBurstTailRow(contentDiv, tailRoot, messageItem) {
  ...
  const avatar = messageItem
    ? messageItem.querySelector("img.chat-avatar")
    : null;
...
function promoteStableBlocksToBurstBubbles(stableBlocksRoot, messageItem) {
  ...
  for (const child of children) {
    if (
      child.classList.contains("burst-row") ||
      child.classList.contains("burst-bubble")
    ) {
  ...
  const blockEls = children.filter(
    (child) =>
...
function isLineOnlyToken(text, tokenStart, tokenLength) {
  const lineStart =
    tokenStart === 0 ? 0 : text.lastIndexOf("\n", tokenStart - 1) + 1;
...
function scanBareDivIslandEnd(text, startIndex) {
  ...
  while (index < text.length) {
    ...
    const shouldPush =
      !isSelfClosing &&
      !HTML_VOID_TAGS.has(name) &&
      HTML_ISLAND_STACK_TAGS.has(name);
    if (!shouldPush) {
      index = tagEnd + 1;
      continue;
    }
    ...
    if (HTML_RAWTEXT_TAGS.has(name)) {
      ...
      if (
        rawTextCloseToken.type === "tag" &&
        rawTextCloseToken.isClosing &&
        rawTextCloseToken.name === name
      ) {
        popHtmlIslandStack(stack, name);
...
function findBareDivIslandStableCutoff(text, startOffset = 0) {
  ...
  while (index < text.length) {
    ...
    if (
      ...
      token.name === "div" &&
      !token.isSelfClosing &&
      isBareDivIslandLineStart(text, tagStart)
    ) {
  ...
  while (index < text.length) {
    ...
    if (startsWithAt(text, index, TOOL_CALL_SUMMARY_START)) {
      const endIndex = text.indexOf(
        ...
        index + TOOL_CALL_SUMMARY_START.length
      );
      if (endIndex === -1) {
        blockedByUnclosedExplicitBlock = true;
        break;
    ...
    if (startsWithAt(text, index, THOUGHT_CHAIN_START)) {
      const endIndex = text.indexOf(
        THOUGHT_CHAIN_END,
        index + THOUGHT_CHAIN_START.length
      );
      if (endIndex === -1) {
        blockedByUnclosedExplicitBlock = true;
        break;
...
function renderStreamFrame(messageId) {
  ...
  const { stableRoot, stableBlocksRoot, tailRoot } =
    ensureStreamingRoots(contentDiv);
  const segmentState = getOrCreateStreamSegmentState(messageId);

  const textForRendering = accumulatedStreamText.get(messageId) || "";
  let nextStableCutoff = findExplicitStablePrefix(
    textForRendering,
  ...
  if (
    (segmentState.burstBubbleCount > 0 ||
      contentDiv.classList.contains("burst-streaming")) &&
    nextStableCutoff > segmentState.stableCutoff &&
    !hasLineOnlyToken(
      textForRendering.slice(segmentState.stableCutoff, nextStableCutoff),
      BURST_MARKER_TOKEN
    )
  ) {
    nextStableCutoff = segmentState.stableCutoff;
  ...
  if (nextStableCutoff > segmentState.stableCutoff) {
    ...
    const newStableText = textForRendering.slice(
      segmentState.stableRenderedCutoff,
      nextStableCutoff
    );
    const hasBurstMarkerInStable = hasLineOnlyToken(
      stableText,
      BURST_MARKER_TOKEN
    );
    const hasBurstMarkerInNewStable = hasLineOnlyToken(
      newStableText,
      BURST_MARKER_TOKEN
    );
    ...
    try {
      ...
      if (hasBurstMarkerInStable || hasBurstMarkerInNewStable) {
        ...
        bubbles = promoteStableBlocksToBurstBubbles(
          stableBlocksRoot,
          messageItem
        );
    ...
    try {
      refs.morphdom(tailRoot, `<div>${rawHtml}</div>`, {
        ...
        onNodeAdded: function (node) {
          ...
          if (
            node.nodeType === 1 &&
            STREAM_BLOCK_TAG_REGEX.test(node.tagName)
          ) {
            // 确保新节点应用横向渐入类
...
export async function startStreamingMessage(message, passedMessageItem = null) {
  ...
  if (
    ...
    messageInitializationStatus.get(messageId) === "ready"
  ) {
    console.debug(
  ...
  if (deferredFinalization) {
    ...
    console.warn(
      `[StreamManager] Replaying deferred finalization for message ${messageId}.`
    );
    setTimeout(() => {
      ...
      if (DESKTOP_PUSH_END_TAG.startsWith(state.tagBuffer)) {
        if (state.tagBuffer === DESKTOP_PUSH_END_TAG) {
          ...
          if (canPush && state.created) {
            if (state.isBuiltinMode) {
              // Builtin 模式：解析完整的 key-value 块，发送 createBuiltinWidget
              try {
                const payload = parseDesktopBuiltinBlock(state.buffer);
                if (payload) {
                  const builtinOptions = payload.options || {};
                  electronAPI.desktopPush({
                    action: "createBuiltinWidget",
                    widgetId: state.widgetId,
                    builtinType: payload.type,
                    config: payload.config,
                    options: {
                      x: builtinOptions.x || 200,
                      y: builtinOptions.y || 150,
                      width: builtinOptions.width || 880,
                      height: builtinOptions.height || 620,
                      ...builtinOptions,
                    },
                  });
                  console.log(
                    `[DesktopPush] Builtin widget sent: type=${payload.type}, widgetId=${state.widgetId}`
                  );
                } else {
                  console.warn(`[DesktopPush] Failed to parse builtin block`);
                }
              } catch (parseErr) {
                console.error(`[DesktopPush] Builtin parse error:`, parseErr);
              }
            } else if (state.isReplaceMode) {
        ...
        if (!state.validated && state.buffer.trim().length >= 5) {
          ...
          if (isValid) {
            ...
            const isBuiltinMode = trimmedBuffer.startsWith("type:");
            state.isReplaceMode = isReplaceMode;
            state.isBuiltinMode = isBuiltinMode;

            if (isBuiltinMode) {
              console.log(
                `[DesktopPush] Builtin mode detected, waiting for complete block...`
              );
              state.created = true; // 标记为已处理，但不创建流式挂件
              // Builtin 模式不需要定时推送，等结束标签时一次性解析
            ...
            // 🟢 替换/Builtin 模式也需要空闲超时保护
            // 这些模式没有 pushTimer，需要单独的超时机制
            if ((state.isReplaceMode || state.isBuiltinMode) && canPush) {
              state.lastTokenTime = Date.now();
...
export async function finalizeStreamedMessage(
  ...
  if (!initStatusAtFinalize || initStatusAtFinalize === "pending") {
    console.warn(
      `[StreamManager] Finalization arrived before message initialization completed for ${messageId}. Deferring. status=${
        initStatusAtFinalize || "missing"
      }`
    );
    pendingFinalizationEvents.set(messageId, {
      finishReason,
      context,
      finalPayload,
    });
  ...
  if (isForCurrentView) {
    ...
    if (messageItem) {
      ...
      if (contentDiv) {
        ...
        const preparedFinal =
          typeof refs.prepareFinalTextForRender === "function"
            ? refs.prepareFinalTextForRender(
                messageId,
                finalFullText,
                message.role || "assistant",
                historyForThisMessage
              )
            : {
                text: finalFullText,
