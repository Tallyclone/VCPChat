...
window.topicListManager = (() => {
  ...
  function init(config) {
    if (!config.elements || !config.elements.topicListContainer) {
      console.error(
        "[TopicListManager] Missing required DOM element: topicListContainer."
      );
      return;
    }
    if (
      ...
      !config.refs ||
      !config.uiHelper ||
      !config.mainRendererFunctions
    ) {
      console.error(
        "[TopicListManager] Missing required configuration parameters."
      );
      return;
    }
  ...
  function shouldActivateCount(history) {
    ...
    return (
      ...
      nonSystemMessages[0].role === "assistant"
    );
  }
  ...
  async function loadTopicList() {
    ...
    if (topicListUl) {
      ...
      const topicsHeader =
        topicListContainer.querySelector(".topics-header") ||
      ...
      if (!topicsHeader.classList.contains("topics-header")) {
        ...
        const newTopicSearchInput =
          topicsHeader.querySelector("#topicSearchInput");
    ...
    if (!currentSelectedItem.id) {
      topicListUl.innerHTML =
        "<li><p>请先在“助手与群组”列表选择一个项目以查看其相关话题。</p></li>";
      return;
    }
    ...
    if (currentSelectedItem.type === "agent") {
      ...
      itemConfigFull = await electronAPI.getAgentGroupConfig(
    ...
    if (!itemConfigFull || itemConfigFull.error) {
      topicListUl.innerHTML = `<li><p>无法加载 ${itemNameForLoading} 的配置信息: ${
        itemConfigFull?.error || "未知错误"
      }</p></li>`;
    } else {
      let topicsToProcess = itemConfigFull.topics || [];
      if (
        currentSelectedItem.type === "agent" &&
        topicsToProcess.length === 0
      ) {
        const defaultAgentTopic = {
          id: "default",
          name: "主要对话",
          createdAt: Date.now(),
        };
      ...
      if (searchTerm) {
        let frontendFilteredTopics = topicsToProcess.filter((topic) => {
          ...
          const nameMatch = normalizedTopicTitle
            .toLowerCase()
            .includes(searchTerm);
          let dateMatch = false;
          if (topic.createdAt) {
            const date = new Date(topic.createdAt);
            const fullDateStr = `${date.getFullYear()}-${String(
              date.getMonth() + 1
            ...
            const shortDateStr = `${date.getFullYear()}-${String(
              date.getMonth() + 1
            ).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
            dateMatch =
              fullDateStr.toLowerCase().includes(searchTerm) ||
              shortDateStr.toLowerCase().includes(searchTerm);
          }
        ...
        try {
          const contentSearchResult = await electronAPI.searchTopicsByContent(
            currentSelectedItem.id,
        ...
        const finalFilteredTopicIds = new Set(
          frontendFilteredTopics.map((t) => t.id)
        );
        contentMatchedTopicIds.forEach((id) => finalFilteredTopicIds.add(id));

        topicsToProcess = topicsToProcess.filter((topic) =>
          finalFilteredTopicIds.has(topic.id)
        );
      }

      if (topicsToProcess.length === 0) {
        topicListUl.innerHTML = `<li><p>${itemNameForLoading} 还没有任何话题${
          searchTerm ? "匹配当前搜索" : ""
        }。您可以点击上方的“新建${
          currentSelectedItem.type === "group" ? "群聊话题" : "聊天话题"
        }”按钮创建一个。</p></li>`;
      } else {
        topicListUl.innerHTML = "";
        ...
        const renderBatch = () => {
          ...
          for (; currentIndex < end; currentIndex++) {
            ...
            avatarImg.src = currentSelectedItem.avatarUrl
              ? currentSelectedItem.avatarUrl
              : currentSelectedItem.type === "group"
              ? "assets/default_group_avatar.png"
              : "assets/default_avatar.png";
            const displayTopicTitle = normalizeTopicTitle(
              topic.name || `话题 ${topic.id}`
            );
            ...
            setTimeout(() => {
              ...
              let historyPromise;
              if (currentSelectedItem.type === "agent") {
                historyPromise = electronAPI.getChatHistory(
                  currentSelectedItem.id,
                  topic.id
                );
              } else if (currentSelectedItem.type === "group") {
                historyPromise = electronAPI.getGroupChatHistory(
                  currentSelectedItem.id,
                  topic.id
                );
              }
                ...
                historyPromise
                  .then((historyResult) => {
                    if (
                      ...
                      const unreadCount = calculateTopicUnreadCount(
                        topic,
                        historyResult
                      );
                      if (unreadCount > 0) {
            ...
            li.addEventListener("click", async () => {
              ...
              try {
                ...
                console.error(
                  "[TopicListManager] Failed to select topic:",
                  error
                );
                if (uiHelper && uiHelper.showToastNotification) {
                  uiHelper.showToastNotification(
                    `打开话题失败: ${error.message}`,
                    "error"
                  );
                }
              }
  ...
  function initializeTopicSortable(itemId, itemType) {
    ...
    if (!topicListUl) {
      console.warn(
        "[TopicListManager] topicListUl element not found. Skipping Sortable initialization."
      );
      return;
    }
    ...
    topicListUl.sortableInstance = new Sortable(topicListUl, {
      ...
      onStart: async function (evt) {
        ...
        if (electronAPI?.getSelectionListenerStatus) {
          wasSelectionListenerActive =
            await electronAPI.getSelectionListenerStatus();
      ...
      onEnd: async function (evt) {
        ...
        try {
          ...
          if (itemType === "agent") {
            ...
            result = await electronAPI.saveGroupTopicOrder(
              itemId,
              orderedTopicIds
            );
          }
          ...
          if (result && result.success) {
            ...
            console.error(
              `Failed to save topic order for ${itemType} ${itemId}:`,
              result?.error
            );
            uiHelper.showToastNotification(
              `保存话题顺序失败: ${result?.error || "未知错误"}`,
              "error"
            );
            loadTopicList();
          }
          ...
          console.error(
            `Error calling saveTopicOrder for ${itemType} ${itemId}:`,
            error
          );
          uiHelper.showToastNotification(
            `调用保存话题顺序API时出错: ${error.message}`,
            "error"
          );
          loadTopicList();
        }
  ...
  function showTopicContextMenu(
    ...
    editTitleOption.onclick = () => {
      closeTopicContextMenu();
      const titleDisplayElement = topicItemElement.querySelector(
        ".topic-title-display"
      );
      ...
      confirmButton.onclick = async (e) => {
        ...
        if (newTitle && newTitle !== originalTitle) {
          let saveResult;
          if (itemType === "agent") {
            saveResult = await electronAPI.saveAgentTopicTitle(
              itemFullConfig.id,
              topic.id,
              newTitle
            );
          } else if (itemType === "group") {
            saveResult = await electronAPI.saveGroupTopicTitle(
              itemFullConfig.id,
              topic.id,
              newTitle
            );
          ...
          if (saveResult && saveResult.success) {
            ...
            if (itemFullConfig.topics) {
              const topicInFullConfig = itemFullConfig.topics.find(
                (t) => t.id === topic.id
              );
            ...
            uiHelper.showToastNotification(
              `更新话题标题失败: ${saveResult?.error || "未知错误"}`,
              "error"
            );
          }
        }
      ...
      topicItemElement.insertBefore(
        inputWrapper,
        titleDisplayElement.nextSibling
      );
      inputField.focus();
      inputField.select();
    ...
    toggleLockOption.onclick = async () => {
      closeTopicContextMenu();
      try {
        const result = await electronAPI.toggleTopicLock(
          itemFullConfig.id,
          topic.id
        );
        if (result.success) {
    ...
    toggleUnreadOption.onclick = async () => {
      closeTopicContextMenu();
      try {
        const result = await electronAPI.setTopicUnread(
          itemFullConfig.id,
          topic.id,
          !isUnread
        );
    ...
    deleteTopicPermanentlyOption.classList.add(
      "context-menu-item",
      "danger-item"
    );
    ...
    deleteTopicPermanentlyOption.onclick = async () => {
      ...
      if (confirmed) {
        ...
        if (itemType === "agent") {
          ...
          result = await electronAPI.deleteGroupTopic(
            itemFullConfig.id,
            topic.id
          );
        }
    ...
    document.addEventListener(
      "click",
      closeTopicContextMenuOnClickOutside,
      true
    );
  }
  ...
  async function handleExportTopic(itemId, itemType, topicId, topicName) {
    ...
    console.log(
      `[TopicListManager] Exporting currently visible topic: ${topicName} (ID: ${topicId})`
    );
    ...
    try {
      ...
      messageItems.forEach((item, index) => {
        if (
          ...
          item.classList.contains("thinking")
        ) {
          console.log(
            `[Export Debug] Skipping system/thinking message at index ${index}.`
          );
          return;
        }
        ...
        if (senderElement && contentElement) {
          ...
          contentClone
            .querySelectorAll(".vcp-thought-chain-bubble")
          ...
          if (sender && content) {
            ...
            console.log(
              `[Export Debug] Skipping message at index ${index} due to empty sender or content. Sender: "${sender}", Content: "${content}"`
            );
          }
        } else {
          console.log(
            `[Export Debug] Skipping message at index ${index} because sender or content element was not found.`
          );
        }
      });

      console.log(
        `[Export Debug] Extracted ${extractedCount} messages. Final markdown length: ${markdownContent.length}`
      );
      ...
      if (result.success) {
        uiHelper.showToastNotification(
          `话题 "${topicName}" 已成功导出到: ${result.path}`
        );
      } else {
        uiHelper.showToastNotification(
          `导出话题失败: ${result.error}`,
    ...
    topicsContainer.addEventListener("click", (e) => {
      if (e.button === 0) {
        ...
        if (timeDiff < 300) {
          ...
          if (
            window.uiManager &&
            typeof window.uiManager.switchToTab === "function"
          ) {
            window.uiManager.switchToTab("settings");
          } else {
            console.warn(
              "[TopicListManager] uiManager不可用，无法切换到设置页面"
            );
          }
        }
      ...
      if (e.button === 1) {
        ...
        if (
          ...
          if (
            ...
            typeof window.itemListManager.resetMouseEventStates === "function"
          ) {
            window.itemListManager.resetMouseEventStates();
          }
        } else {
          console.warn(
            "[TopicListManager] uiManager不可用，无法切换到助手页面"
          );
        }
      }
    });
