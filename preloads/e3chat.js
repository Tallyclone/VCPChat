...
function invoke(channel, ...args) {
  return ipcRenderer.invoke(channel, ...args).then((result) => {
    if (!result?.success) throw new Error(result?.error || "E3 Chat IPC failed");
    return result.data;
  });
}
...
const api = {
  ...
  renameSession: (sessionId, title) => invoke("e3chat:rename-session", sessionId, title),
  ...
  saveLocalState: (scope, patch) => invoke("e3chat:save-local-state", scope, patch),
  onEvent: (listener) => {
