...
const {
  ...
  Tray,
  Menu,
} = require("electron"); // Added screen, clipboard, and shell
...
const {
  getAgentConfigById,
  ...agentHandlers
} = require("./modules/ipc/agentHandlers"); // Import agent handlers
...
const nativeFsHandlers = require("./modules/ipc/nativeFsHandlers"); // Import NativeFileMount filesystem handlers
const {
  PRELOAD_ROLES,
  resolveProjectPreload,
} = require("./modules/services/preloadPaths");
...
const fileWatcher = {
  watchFile: (filePath, callback) => {
    ...
    historyWatcher.on("all", (event, path) => {
      // 🔧 改进：使用时间窗口而非一次性标志
      const now = Date.now();
      const isWithinSaveWindow =
        now - lastInternalSaveTime < INTERNAL_SAVE_WINDOW_MS;

      if (isWithinSaveWindow || isEditingInProgress) {
        console.log(
          `[FileWatcher] Ignored ${
            isWithinSaveWindow ? "internal save" : "editing"
          } event '${event}' for: ${path} (time since last save: ${
            now - lastInternalSaveTime
          }ms)`
        );
        return;
      }
      console.log(
        `[FileWatcher] Detected external event '${event}' for: ${path}`
      );
      callback(path);
    });
  ...
  signalInternalSave: () => {
    ...
    console.log(
      "[FileWatcher] Internal save signaled at:",
      lastInternalSaveTime
    );
...
const MUSIC_PLAYLIST_FILE = path.join(
  APP_DATA_ROOT_IN_PROJECT,
  "songlist.json"
);
const MUSIC_COVER_CACHE_DIR = path.join(
  APP_DATA_ROOT_IN_PROJECT,
  "MusicCoverCache"
);
const NETWORK_NOTES_CACHE_FILE = path.join(
  APP_DATA_ROOT_IN_PROJECT,
  "network-notes-cache.json"
); // Cache for network notes
const WALLPAPER_THUMBNAIL_CACHE_DIR = path.join(
  APP_DATA_ROOT_IN_PROJECT,
...
let distributedServerConfig = null; // Cached distributed server settings for local plugin calls
...
function startAudioEngine() {
  return new Promise((resolve, reject) => {
    ...
    const binaryName =
      process.platform === "win32" ? "audio_server.exe" : "audio_server";
    ...
    try {
      ...
      try {
        ...
        if (error.name !== "AbortError") {
          console.warn(
            `[Main] Audio Engine shutdown request failed: ${error.message}`
          );
        }
      } finally {
      ...
      if (audioEngineProcess === processRef && !processRef.killed) {
        console.warn(
          "[Main] Audio Engine did not exit after graceful shutdown request. Force killing process."
        );
        processRef.kill();
        await Promise.race([
          exitPromise,
...
function createWindow() {
  ...
  mainWindow.on("close", (event) => {
    ...
    if (dw && !dw.isDestroyed()) {
      ...
      mainWindow.hide();
      console.log(
        "[Main] Desktop window active — main window hidden to tray instead of closing."
      );
    }
  ...
  mainWindow.webContents.on(
    ...
    (event, errorCode, errorDescription, validatedURL) => {
      console.error(
        "[Main] Main window did-fail-load",
        errorCode,
        errorDescription,
        validatedURL
      );
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    ipcMain.on("close-app", () => {
      console.log(
        "[Main] Received close-app request from splash screen. Quitting."
      );
      app.quit();
    });
    ...
    groupChat.initializePaths({
      APP_DATA_ROOT_IN_PROJECT,
      AGENT_DIR,
      USER_DATA_DIR,
      SETTINGS_FILE,
    }); // Initialize GroupChat paths
    ...
    async function fetchAndCacheModels() {
      ...
      try {
        ...
        if (!vcpServerUrl) {
          console.warn(
            "[Main] VCP Server URL is not configured. Cannot fetch models."
          );
    ...
    fetchAndCacheModels()
      .then(() => {
        if (mainWindow && !mainWindow.isDestroyed()) {
          console.log(
            "[Main] Background model fetch complete. Notifying renderer."
          );
    ...
    ipcMain.handle("refresh-models", async (event) => {
      console.log(
        "[Main] Received refresh-models request. Re-fetching models..."
      );
    ...
    const TRANSLATOR_DIR = path.join(
      APP_DATA_ROOT_IN_PROJECT,
      "Translatormodules"
    );
    ...
    ipcMain.handle("open-translator-window", async (event) => {
      ...
      try {
        ...
        console.error(
          "Failed to read settings file for translator window:",
          readError
        );
      }
      ...
      const translatorUrl = `file://${path.join(
        __dirname,
        "Translatormodules",
        "translator.html"
      )}?vcpServerUrl=${encodeURIComponent(
        vcpServerUrl
      )}&vcpApiKey=${encodeURIComponent(vcpApiKey)}`;
      console.log(
        `[Main Process] Attempting to load URL in translator window: ${translatorUrl.substring(
          0,
          200
        )}...`
      );
      ...
      translatorWindow.webContents.on("did-start-loading", () => {
        console.log(
          `[Main Process] translatorWindow webContents did-start-loading for URL: ${translatorUrl.substring(
            0,
            200
          )}`
        );
      ...
      translatorWindow.webContents.on("dom-ready", () => {
        console.log(
          `[Main Process] translatorWindow webContents dom-ready for URL: ${translatorWindow.webContents.getURL()}`
        );
      });

      translatorWindow.webContents.on("did-finish-load", () => {
        console.log(
          `[Main Process] translatorWindow webContents did-finish-load for URL: ${translatorWindow.webContents.getURL()}`
        );
      });

      translatorWindow.webContents.on(
        "did-fail-load",
        (event, errorCode, errorDescription, validatedURL) => {
          console.error(
            `[Main Process] translatorWindow webContents did-fail-load: Code ${errorCode}, Desc: ${errorDescription}, URL: ${validatedURL}`
          );
        }
      );

      translatorWindow
        .loadURL(translatorUrl)
        .then(() => {
          console.log(
            `[Main Process] translatorWindow successfully initiated URL loading (loadURL resolved): ${translatorUrl.substring(
              0,
              200
            )}`
          );
        ...
        .catch((err) => {
          console.error(
            `[Main Process] translatorWindow FAILED to initiate URL loading (loadURL rejected): ${translatorUrl.substring(
              0,
              200
            )}`,
            err
      ...
      translatorWindow.once("ready-to-show", () => {
        console.log(
          `[Main Process] translatorWindow is ready-to-show. Window Title: "${translatorWindow.getTitle()}". Calling show().`
        );
        translatorWindow.show();
      ...
      translatorWindow.on("closed", () => {
        ...
        openChildWindows = openChildWindows.filter(
          (win) => win !== translatorWindow
        );
      ...
      if (fileWatcher) {
        fileWatcher.watchFile(filePath, (changedPath) => {
          if (mainWindow && !mainWindow.isDestroyed()) {
            ...
            mainWindow.webContents.send("history-file-updated", {
              path: changedPath,
              agentId,
              topicId,
            });
    ...
    nativeFsHandlers.initialize({
      getDesktopWindow: () => desktopHandlers.getDesktopWindow(),
    });
    ...
    (async () => {
      try {
        ...
        if (settings.enableDistributedServer) {
          ...
          const config = {
            ...
            handleDesktopRemoteControl:
              desktopRemoteHandlers.handleDesktopRemoteControl, // Inject the desktop remote control handler
          };
        ...
        distributedServer = null;
        distributedServerConfig = null;
        console.error(
          "[Main] Failed to read settings or initialize distributed server:",
          error
        );
      }
    })();
    ...
    if (isAutoOpenDesktop) {
      console.log(
        "[Main] --desktop-only flag detected. Auto-opening desktop window after full initialization."
      );
      // 延迟打开，确保主窗口已完全就绪
      setTimeout(async () => {
        await desktopHandlers.openDesktopWindow();
  ...
  ipcMain.handle("execute-python-code", (event, code) => {
    return new Promise((resolve) => {
      ...
      pythonProcess.on("error", (err) => {
        ...
        resolve({
          stdout: "",
          stderr: `Failed to start python process. Please ensure Python is installed and accessible in your system's PATH. Error: ${err.message}`,
        });
      });
  ...
  app.on("before-quit", async (event) => {
    ...
    try {
      await nativeFsHandlers.closeAll();
  ...
  function connectVcpLog(wsUrl, wsKey) {
    ...
    if (!wsUrl || !wsKey) {
      if (mainWindow)
        mainWindow.webContents.send("vcp-log-status", {
          source: "VCPLog",
          status: "error",
          message: "URL或KEY未配置。",
        });
    ...
    if (
      vcpLogWebSocket &&
      (vcpLogWebSocket.readyState === WebSocket.OPEN ||
        vcpLogWebSocket.readyState === WebSocket.CONNECTING)
    ) {
    ...
    vcpLogWebSocket.onopen = () => {
      ...
      if (
        ...
        console.log(
          '[MAIN_VCP_LOG] Attempting to send vcp-log-status "open" to renderer.'
        );
        mainWindow.webContents.send("vcp-log-status", {
          source: "VCPLog",
          status: "open",
        ...
        mainWindow.webContents.send("vcp-log-message", {
          type: "connection_ack",
          message: "VCPLog 连接成功！",
        });
      } else {
        console.error(
          "[MAIN_VCP_LOG] mainWindow or webContents not available in onopen. Cannot send status."
        );
      }
      if (vcpLogReconnectInterval) {
    ...
    vcpLogWebSocket.onmessage = (event) => {
      ...
      try {
        ...
        if (mainWindow && !mainWindow.isDestroyed())
          mainWindow.webContents.send("vcp-log-message", data);
      } catch (e) {
        console.error("VCPLog 解析消息失败:", e);
        if (mainWindow && !mainWindow.isDestroyed())
          mainWindow.webContents.send("vcp-log-message", {
            type: "error",
            data: `收到无法解析的消息: ${event.data
              .toString()
              .substring(0, 100)}...`,
          });
      }
    ...
    vcpLogWebSocket.onerror = (error) => {
      ...
      if (
        ...
        console.error(
          "[MAIN_VCP_LOG] mainWindow or webContents not available in onerror."
        );
      }
    };
  }
...
ipcMain.on("open-voice-chat-window", (event, { agentId }) => {
  ...
  voiceChatWindow.loadFile(
    path.join(__dirname, "Voicechatmodules/voicechat.html")
  );
...
ipcMain.on("start-speech-recognition", async (event) => {
  const voiceChatWindow = openChildWindows.find(
  ...
  try {
    ...
    speechConfig = {
      ...
      recognizerPagePath:
        settings?.speechRecognizerPagePath ||
        "Voicechatmodules/recognizer.html",
    };
  } catch (error) {
    console.warn(
      "[Main] Failed to read speech recognition settings, using defaults:",
      error.message
    );
  }
...
ipcMain.handle("export-topic-as-markdown", async (event, exportData) => {
  ...
  const defaultFileName = `${safeTopicName}-${formatTimestampForFilename(
    Date.now()
  )}.md`;
...
ipcMain.handle("interrupt-group-request", (event, messageId) => {
  console.log(
    `[Main] Received interrupt-group-request for messageId: ${messageId}`
  );
  if (groupChat && typeof groupChat.interruptGroupRequest === "function") {
    return groupChat.interruptGroupRequest(messageId);
  } else {
    console.error(
      "[Main] groupChat module or interruptGroupRequest function is not available."
    );
    return {
      success: false,
      error: "Group chat module not initialized correctly.",
    };
  }
});
