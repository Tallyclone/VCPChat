...
if (!gotTheLock) {
  ...
  const isInternalLaunch =
    process.argv.includes("--desktop-only") ||
    process.argv.includes("--rag-observer-only");
  ...
  if (!isInternalLaunch) {
    ...
    try {
      ...
      console.warn(
        "[Main] Failed to create .vcp_ready in second instance:",
        err.message
      );
    }
  }
    ...
    setTimeout(() => {
      try {
        if (fs.existsSync(readyFile)) {
          fs.unlinkSync(readyFile);
          console.log(
            "[Main] Cleaned up .vcp_ready signal created by second instance."
          );
        }
      } catch (err) {
        console.warn(
          "[Main] Failed to clean up second-instance .vcp_ready:",
          err.message
        );
      }
    }, 1500);
  ...
  app.whenReady().then(async () => {
    ...
    const noteMiniShortcutRegistered = globalShortcut.register(
      "Super+Alt+Z",
      () => {
