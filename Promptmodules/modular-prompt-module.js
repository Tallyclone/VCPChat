...
class ModularPromptModule {
  ...
  async loadData() {
    ...
    let privateOrder = this.warehouseOrder.filter(
      (w) => w !== "global" && w !== "default"
    );
    // 以固定的顺序重建
  ...
  createToolbar() {
    ...
    addBtn.innerHTML =
      '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg><span>积木块</span>';
    addBtn.title = "添加积木块";
    ...
    addNewlineBtn.innerHTML =
      '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M3 6h18M3 12h18M3 18h12"/></svg><span>换行</span>';
  ...
  createBlockElement(block, index) {
    ...
    if (block.type === "newline") {
      ...
      const displayText =
        block.name && block.name.trim() ? block.name : currentContent;
      ...
      if (block.variants && block.variants.length > 1) {
        ...
        indicator.title = `此积木块有 ${
          block.variants.length
        } 个内容条目，当前为第 ${(block.selectedVariant || 0) + 1} 个`;
    ...
    blockEl.addEventListener("dragstart", (e) =>
      this.handleDragStart(e, block, index)
    );
    blockEl.addEventListener("dragover", (e) =>
      this.handleDragOver(e, blockEl, index)
    );
    blockEl.addEventListener("dragleave", (e) =>
      this.handleDragLeave(e, blockEl)
    );
  ...
  showBlockContextMenu(e, block, index) {
    ...
    if (
      ...
      block.variants.forEach((variant, variantIndex) => {
        const preview =
          variant.substring(0, 30) + (variant.length > 30 ? "..." : "");
        menuItems.push({
          label: `${
            variantIndex === (block.selectedVariant || 0) ? "✓ " : ""
          }${preview}`,
          action: () => {
  ...
  editBlock(block, index) {
    ...
    let dialogHTML = `
            ...
            <div class="dialog-content">
                ...
                <div class="dialog-field">
                    <label>名称（可选）:</label>
                    <input type="text" class="block-name-input" value="${
                      block.name || ""
                    }" placeholder="为积木块命名...">
                </div>
    ...
    block.variants.forEach((variant, idx) => {
      dialogHTML += `
                        <div class="variant-item-edit" data-index="${idx}">
                            <textarea class="variant-content-input" rows="3" placeholder="内容条目 ${
                              idx + 1
                            }">${variant}</textarea>
                            ${
                              block.variants.length > 1
                                ? `<button class="remove-variant-btn" data-index="${idx}">×</button>`
                                : ""
                            }
                        </div>`;
    });
    ...
    saveBtn.onclick = () => {
      ...
      // 收集所有内容条目
      const variantInputs = variantsList.querySelectorAll(
      ...
      if (
        !block.selectedVariant ||
        block.selectedVariant >= block.variants.length
      ) {
        block.selectedVariant = 0;
      }
  ...
  copyBlockToWarehouse(index) {
    ...
    const isDuplicate = this.hiddenBlocks[this.currentWarehouse].some(
      (hiddenBlock) => {
  ...
  renderWarehouse() {
    ...
    addWarehouseBtn.innerHTML =
      '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg><span>新建小仓</span>';
    ...
    this.warehouseOrder.forEach((name, index) => {
      ...
      if (name !== "default" && name !== "global") {
        ...
        warehouseItem.addEventListener("dragstart", (e) =>
          this.handleWarehouseDragStart(e, name, index)
        );
        warehouseItem.addEventListener("dragover", (e) =>
          this.handleWarehouseDragOver(e, index)
        );
        warehouseItem.addEventListener("drop", (e) =>
          this.handleWarehouseDrop(e, index)
        );
        warehouseItem.addEventListener("dragend", () =>
          this.handleWarehouseDragEnd()
        );
      }
    ...
    if (currentHidden.length === 0) {
      hiddenBlocksList.innerHTML =
        '<div class="warehouse-empty">此仓库为空<br><small style="font-size:0.85em;opacity:0.7;">拖拽积木块到这里</small></div>';
    } else {
  ...
  createHiddenBlockElement(block, index) {
    ...
    // 显示名称或内容预览
    const displayText =
      block.name || (block.content ? block.content : "[空积木块]");
   ...
   */
  removeDropIndicator() {
    const targets = this.blocksContainer.querySelectorAll(
      ".drop-target, .drop-target-left, .drop-target-right"
    );
    targets.forEach((el) => {
  ...
  async save() {
    ...
    const globalBlocksToSave = structuredClone(
      this.hiddenBlocks["global"] || []
    );
  ...
  deleteWarehouse(warehouseName) {
    if (
      !confirm(`确定要删除仓库 "${warehouseName}" 吗？其中的积木块也会被删除。`)
    ) {
      return;
    }

    delete this.hiddenBlocks[warehouseName];
    this.warehouseOrder = this.warehouseOrder.filter(
      (w) => w !== warehouseName
    );
