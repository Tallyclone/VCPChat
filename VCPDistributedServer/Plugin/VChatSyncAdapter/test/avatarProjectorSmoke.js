...
async function main() {
  ...
  const settingsUserAvatarResult = await projectEvents(
    [
      {
        seq: 9,
        operation_id: "avatar-settings-user-path",
        entity_type: "avatar",
        ...
        payload: {
          ...
          hash: avatarHashV1,
          ext: ".png",
          relative_path: "assets/profile.png",
          metadata: { source: "settings.userAvatarUrl" },
        },
      },
    ],
    { config, localIndex, writeIntentLock, logger, centerClient }
  );
  assert.deepStrictEqual(settingsUserAvatarResult.appliedSeqs, [9]);
  assert.strictEqual(
    await fs.readFile(
      path.join(config.appDataPath, "assets", "profile.png"),
      "utf8"
    ),
    "avatar-v1"
  );
  assert.strictEqual(
    localIndex.getFile("assets/profile.png").hash,
    avatarHashV1
  );
