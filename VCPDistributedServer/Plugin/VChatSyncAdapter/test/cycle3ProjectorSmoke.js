...
async function main() {
  ...
  const initialAgentConfigPath = path.join(
    config.appDataPath,
    "Agents",
    "agent-one",
    "config.json"
  );
  await fs.outputJson(initialAgentConfigPath, {
    name: "agent-one",
    topics: [{ id: "topic-one", name: "Topic One", createdAt: 1 }],
  });
  ...
  assert.strictEqual(
    localIndex.getTopicSnapshot("agent:agent-one:topic-one"),
    null
  );
    ...
    [
      {
        ...
        payload: {
          topic: {
            ...
            item_id: "topic-agent",
            item_type: "agent",
            owner_id: "topic-agent",
            owner_type: "agent",
            topic_id: "topic-b",
            nameSource: "center",
            nameGeneratedAt: "2026-06-08T00:00:00.000Z",
            nameGeneratedDeviceId: "device-x",
            order_rank: 2000,
            order_version: 100,
            order_updated_at: "2026-06-08T00:00:00.000Z",
          },
        },
      },
    ],
  ...
  assert.strictEqual(updatedTopicB.customNewField, undefined);
  assert.strictEqual(updatedTopicB.item_id, undefined);
  assert.strictEqual(updatedTopicB.item_type, undefined);
  assert.strictEqual(updatedTopicB.owner_id, undefined);
  assert.strictEqual(updatedTopicB.owner_type, undefined);
  assert.strictEqual(updatedTopicB.topic_id, undefined);
  assert.strictEqual(updatedTopicB.nameSource, undefined);
  assert.strictEqual(updatedTopicB.nameGeneratedAt, undefined);
  assert.strictEqual(updatedTopicB.nameGeneratedDeviceId, undefined);
  assert.strictEqual(updatedTopicB.order_rank, 2000);
  assert.strictEqual(updatedTopicB.order_version, 100);
  assert.strictEqual(
    updatedTopicB.order_updated_at,
  ...
  const topicCenterOrderResult = await projectEvents(
    [
      {
        seq: 18,
        operation_id: "op-topic-order-center-rank",
        entity_type: "topic_order",
        entity_id: "topic-agent",
        action: "move",
        item_type: "agent",
  ...
  assert.deepStrictEqual(topicCenterOrderResult.appliedSeqs, [18]);
  topicConfig = await fs.readJson(topicConfigPath);
  assert.deepStrictEqual(
    topicConfig.topics.map((topic) => topic.id),
    ["topic-c", "topic-b", "topic-a"]
  );
  const orderedTopicC = topicConfig.topics.find(
    (topic) => topic.id === "topic-c"
  );
  assert.strictEqual(orderedTopicC.deepMemoFlag, "keep-c");
  assert.strictEqual(orderedTopicC.order_rank, -1000000);
  assert.strictEqual(orderedTopicC.order_version, 101);
