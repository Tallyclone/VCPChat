...
async function readState(config, logger) {
  try {
    ...
    if (await fs.pathExists(config.statePath)) await fs.move(config.statePath, corruptPath, { overwrite: true });
    logger.error('state.json corrupt; entering recovering mode', { corruptPath, error: error.message });
    const state = { schema_version: 1, mode: 'recovering', enabled: false, last_applied_seq: 0, recovered_at: new Date().toISOString() };
...
async function writeState(config, state) {
  ...
  await fs.writeJson(tmp, { ...state, updated_at: new Date().toISOString() }, { spaces: 2 });
