...
function collectAttachmentRefs(message) {
  ...
  const push = (value) => {
    ...
    let hash = value.hash || value.attachment_hash || value.sha256;
    if (!hash && value._fileManagerData && typeof value._fileManagerData === "object") {
      hash = value._fileManagerData.hash || value._fileManagerData.sha256;
    }
    if (!hash && (value.src || value.internalPath || value.localPath || value.internal_path || value.local_path)) {
      const filePath = value.src || value.internalPath || value.localPath || value.internal_path || value.local_path;
...
async function rewriteMessageAttachments(message, context) {
  ...
  const rewriteObject = (value) => {
    ...
    if (!hash && value._fileManagerData && typeof value._fileManagerData === "object") {
      hash = String(value._fileManagerData.hash || value._fileManagerData.sha256 || "").toLowerCase();
    }
    if (!hash && (value.src || value.internalPath || value.localPath || value.internal_path || value.local_path)) {
      const filePath = value.src || value.internalPath || value.localPath || value.internal_path || value.local_path;
      if (typeof filePath === "string") {
        ...
        const nameWithoutExt = basename.split(".")[0];
        if (/^[a-f0-9]{64}$/i.test(nameWithoutExt)) {
          hash = nameWithoutExt.toLowerCase();
        }
      }
    }
...
async function uploadLocalAttachment(
  ...
  if (
    avatarIdentity &&
    avatarIdentity.owner_type === "user" &&
    avatarIdentity.owner_id !== "user_avatar"
  ) {
