...
const GENERATED_LISTS_CONFIG_PATH = path.join(
  __dirname,
  "..",
  "AppData",
  "generated_lists",
...
const MUSIC_PLAYLIST_FILE_PATH = path.join(
  __dirname,
  "..",
  "AppData",
  "songlist.json"
...
function loadGeneratedListsConfig() {
  try {
    ...
    console.error(
      "[DistributedServer] Failed to read generated_lists/config.env:",
      error.message
    );
    return {};
  }
...
class DistributedServer {
  ...
  async bindHttpServer(preferredPort) {
    ...
    try {
      ...
      if (error && error.code === "EADDRINUSE" && preferredPort !== 0) {
        console.warn(
          `[${this.serverName}] Port ${preferredPort} is already in use. Falling back to a random available port.`
        );
        return tryListen(0);
      }
      throw error;
  ...
  async initialize() {
    ...
    try {
      if (fsSync.existsSync(serverConfigPath)) {
        ...
        if (serverEnv.ServerName && serverEnv.ServerName.trim()) {
          this.serverName = serverEnv.ServerName.trim();
        }
        ...
        if (serverEnv.DIST_SERVER_PORT) {
          ...
          if (!isNaN(newPort)) {
            this.port = newPort;
            console.log(
              `[${this.serverName}] Port loaded from config.env: ${this.port}`
            );
          }
        }
      }
    ...
    console.log(
      `[${this.serverName}] HTTP server listening on 0.0.0.0:${this.port}`
    );
    ...
    this.app.post("/plugin/callback", (req, res) => {
      ...
      if (this.debugMode)
        console.log(
          `[${this.serverName}] Received plugin callback:`,
          callbackData
        );
      ...
      res
        ...
        .json({
          status: "success",
          message: "Callback forwarded to main server.",
        });
    });
  ...
  registerDiagnosticRoutes() {
    ...
    if (!fileKey) {
      console.error(
        `[${this.serverName}] DesktopRemote test route disabled: missing file_key in AppData/generated_lists/config.env.`
      );
      return;
    }
    ...
    this.app.post(/\/pw=([^\/]+)\/desktop-remote-test/, async (req, res) => {
      ...
      let commandPayload;
      try {
        const normalized = await pluginManager.processToolCall(
          "DesktopRemote",
          req.body || {}
        );
        commandPayload =
          typeof normalized === "string" ? JSON.parse(normalized) : normalized;
      } catch (error) {
      ...
      try {
        ...
        if (!result || result.status !== "success") {
          const message =
            result?.message || result?.error || "DesktopRemote handler failed.";
          const stage = /timed out/i.test(message)
            ? "renderer-timeout"
            : "main-handler";
    ...
    console.log(
      `[${this.serverName}] DesktopRemote test route enabled: /pw=<file_key>/desktop-remote-test`
    );
  }
  ...
  connect() {
    ...
    if (!this.mainServerUrl || !this.vcpKey) {
      console.error(
        `[${this.serverName}] Error: mainServerUrl or vcpKey is not configured. Cannot connect.`
      );
      return;
    }

    const connectionUrl = `${this.mainServerUrl.replace(
      /^http/,
      "ws"
    )}/vcp-distributed-server/VCP_Key=${this.vcpKey}`;
    console.log(
      `[${this.serverName}] Attempting to connect to main server at ${connectionUrl}`
    );
    ...
    this.ws.on("open", async () => {
      console.log(
        `[${this.serverName}] Successfully connected to main server.`
      );
      this.reconnectInterval = 5000;
      this.registerTools();
    ...
    this.ws.on("error", (error) => {
      console.error(
        `[${this.serverName}] WebSocket client error:`,
        error.message
      );
  ...
  scheduleReconnect() {
    ...
    console.log(
      `[${this.serverName}] Attempting to reconnect in ${
  ...
  registerTools() {
    ...
    if (manifests.length > 0) {
      ...
      this.sendMessage(payload);
      console.log(
        `[${this.serverName}] Sent registration for ${manifests.length} tools to the main server.`
      );
    } else {
      if (this.debugMode)
        console.log(`[${this.serverName}] No local tools found to register.`);
    }
  }
  ...
  async reportIPAddress() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          `[${this.serverName}] Failed to fetch public IP, status: ${response.status}`
        );
      }
    } catch (e) {
      console.error(
        `[${this.serverName}] Could not fetch public IP:`,
        e.message
      );
    }
    ...
    console.log(
      `[${
        this.serverName
      }] Reported IP addresses to main server: Local: ${ipv4Addresses.join(
        ", "
      )}, Public: ${publicIp || "N/A"}`
    );
  }
  ...
  setupStaticPlaceholderUpdates() {
    ...
    if (this.debugMode)
      console.log(
        `[${this.serverName}] Static placeholder updates scheduled every 30 seconds.`
      );
  }
  ...
  clearStaticPlaceholderUpdates() {
    if (this.staticPlaceholderUpdateInterval) {
      ...
      if (this.debugMode)
        console.log(
          `[${this.serverName}] Static placeholder update interval cleared.`
        );
    }
  }
  ...
  async pushStaticPlaceholderValues() {
    ...
    if (this.debugMode && logStaticPlugins) {
      console.log(
        `[${this.serverName}] Pushed ${placeholderValues.size} static placeholder values to main server.`
      );
  ...
  setupMusicPlaylistWatcher() {
    ...
    const schedulePush = (reason) => {
      ...
      this.musicPlaylistUpdateTimeout = setTimeout(() => {
        ...
        this.pushMusicPlaylistUpdate(reason).catch((error) => {
          console.error(
            `[${this.serverName}] Failed to push music playlist update:`,
            error.message
          );
        });
      }, 500);
    ...
    this.musicPlaylistWatcher
      ...
      .on("error", (error) => {
        console.error(
          `[${this.serverName}] Music playlist watcher error:`,
          error.message
        );
      });

    console.log(
      `[${this.serverName}] Watching local music playlist: ${MUSIC_PLAYLIST_FILE_PATH}`
    );
  }
  ...
  async readMusicPlaylistSnapshot() {
    ...
    const tracks = parsed
      ...
      .map((track) => ({
        ...
        serverId:
          typeof track.serverId === "string" ? track.serverId : undefined,
      }));
  ...
  async pushMusicPlaylistUpdate(reason = "manual") {
    ...
    try {
      ...
      console.error(
        `[${this.serverName}] Failed to read local music playlist:`,
        error.message
      );
      snapshot = {
    ...
    if (
      signature === this.lastMusicPlaylistSignature &&
      reason !== "websocket_open"
    ) {
      if (this.debugMode)
        console.log(
          `[${this.serverName}] Music playlist unchanged, skip update.`
        );
      return;
    }
    ...
    this.sendMessage(payload);
    console.log(
      `[${this.serverName}] Reported local music playlist to main server. Count: ${snapshot.count}, Reason: ${reason}`
    );
  }
  ...
  async closeMusicPlaylistWatcher() {
    ...
    if (this.musicPlaylistWatcher) {
      ...
      await watcher.close();
      if (this.debugMode)
        console.log(`[${this.serverName}] Music playlist watcher closed.`);
    }
  }
  ...
  async shouldLogStaticPlugins() {
    try {
      ...
      if (this.debugMode)
        console.warn(
          `[${this.serverName}] Error reading settings for log control:`,
          error.message
        );
  ...
  async handleMainServerMessage(message) {
    try {
      ...
      if (this.debugMode)
        console.log(
          `[${this.serverName}] Received message from main server:`,
          parsedMessage.type
        );
      ...
      console.error(
        `[${this.serverName}] Error parsing message from main server:`,
        e
      );
    }
  }
  ...
  async handleToolExecutionRequest(data) {
    ...
    if (!requestId || !toolName) {
      console.error(
        `[${this.serverName}] Invalid tool execution request received.`
      );
      return;
    }
    ...
    if (this.debugMode)
      console.log(
        `[${this.serverName}] Executing tool '${toolName}' for request ID: ${requestId}`
      );
    ...
    try {
      ...
      if (toolName === "internal_request_file") {
        ...
        try {
          ...
          if (e.code === "ENOENT") {
            ...
            throw new Error(
              `Invalid file URL path on distributed server: ${fileUrl}`
            );
          } else {
            throw new Error(
              `Error reading file on distributed server (${fileUrl}): ${e.message}`
            );
          }
        }
        ...
        if (this.debugMode)
          console.log(
            `[${this.serverName}] Sent file content for request ID: ${requestId}`
          );
        return; // 处理完毕，直接返回
      }
      // --- 结束：处理内部文件请求 ---
      ...
      if (toolName === "MusicController") {
        ...
        if (typeof this.handleMusicControl !== "function") {
          throw new Error(
            "Music control handler is not configured for the Distributed Server."
          );
        }
        ...
        if (typeof this.handleDiceControl !== "function") {
          throw new Error(
            "Dice control handler is not configured for the Distributed Server."
          );
        }
        ...
        if (typeof this.handleFlowlockControl !== "function") {
          throw new Error(
            "Flowlock control handler is not configured for the Distributed Server."
          );
        }
        ...
        if (typeof this.handleDesktopRemoteControl !== "function") {
          throw new Error(
            "Desktop remote control handler is not configured for the Distributed Server."
          );
        }

        // Directly call the injected handler function from main.js
        const resultFromMain = await this.handleDesktopRemoteControl(
          commandPayload
        );
        ...
        finalResult = resultFromMain.result || {
          message: resultFromMain.message,
        };
      } else {
        ...
        if (typeof result === "object" && result !== null) {
          ...
          try {
            ...
            if (parsedPluginResult.status === "success") {
              ...
              throw new Error(
                parsedPluginResult.error ||
                  "Plugin reported an error without a message."
              );
            }
          } catch (e) {
        ...
        if (finalResult && finalResult._specialAction === "create_canvas") {
          if (typeof this.handleCanvasControl === "function") {
            console.log(
              `[${this.serverName}] Detected create_canvas action. Calling main process handler.`
            );
            this.handleCanvasControl(finalResult.payload.filePath);
          } else {
            console.error(
              `[${this.serverName}] Canvas control handler is not configured for the Distributed Server.`
            );
          }
        }
      ...
      console.error(
        `[${this.serverName}] Error executing tool '${toolName}':`,
        error.message
      );
      responsePayload = {
        type: "tool_result",
    ...
    if (this.debugMode)
      console.log(
        `[${this.serverName}] Sent result for request ID: ${requestId}`
      );
  }
  ...
  sendMessage(payload) {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      console.error(
        `[${this.serverName}] Cannot send message, WebSocket is not open.`
      );
    }
  }
  ...
  async stop() {
    ...
    this.stopPromise = Promise.resolve().then(async () => {
      ...
      pluginManager.shutdownAllPlugins().catch((err) => {
        console.error(
          `[${this.serverName}] Error during plugin shutdown:`,
          err
        );
      });
