...
(function () {
  ...
  function applyWidgetFrameOptions(widgetData, frame = {}) {
    ...
    if (frame.transparent === true) {
      widgetEl.style.background = "transparent";
      widgetEl.style.border = "none";
      widgetEl.style.boxShadow = "none";
    }
    if (frame.background != null)
      widgetEl.style.background = String(frame.background);
    if (frame.border != null) widgetEl.style.border = String(frame.border);
    if (frame.shadow != null) widgetEl.style.boxShadow = String(frame.shadow);
    if (frame.radius != null)
      widgetEl.style.borderRadius = addCssUnit(frame.radius);
    if (frame.opacity != null) widgetEl.style.opacity = String(frame.opacity);
    if (frame.backdropFilter != null)
      widgetEl.style.backdropFilter = String(frame.backdropFilter);

    if (frame.showGrip === false && grip) grip.style.display = "none";
    if (frame.showCloseButton === false && closeBtn)
      closeBtn.style.display = "none";
    if (frame.contentOverflow != null && contentWrapper) {
      contentWrapper.style.overflow = String(frame.contentOverflow);
    }
    if (frame.dragRegion === "none" && grip) {
      grip.style.pointerEvents = "none";
      grip.style.display = "none";
    }
    widgetData.frameOptions = { ...frame };
  }
  ...
  function createWidget(widgetId, options = {}) {
    ...
    widget.addEventListener("contextmenu", (e) => {
      if (options.frame?.contextMenu === false) return;
      e.preventDefault();
      e.stopPropagation();
    ...
    applyWidgetFrameOptions(widgetData, options.frame);
  ...
  function removeWidget(widgetId) {
    ...
    if (typeof widgetData._cleanup === "function") {
      Promise.resolve(widgetData._cleanup()).catch((error) => {
        console.warn(`[Desktop] Widget cleanup failed for ${widgetId}:`, error);
      });
    }
